...
def Updated_wires (grid_start , grid_end , spacing , wire_width, num_layers ,new_widths):
    ...
    wires , num_wires=generate_wires(num_layers,wire_width,spacing,grid_start,grid_end)
        ...
        for wire in wires :
            ...
            if layer == layer_idx-1 :
                ...
                if (y1 == grid_y1 )and ( y2 == grid_y2 ):  # vertical wire
                    ...
                    i+=1
                    if (new_start[0]-old_end[0])< Min_Spacing :
                        shift= abs(new_start[0]-old_end[0]- Min_Spacing)
                        new_start = (Center - new_width+shift ,grid_x1)
                        new_end = (new_width + Center+shift, grid_y2+width_incr_H)
                    ...
                    j+=1
                    if (new_start[1]-old_end[1])< Min_Spacing :
                        shift= abs(new_start[1]-old_end[1]- Min_Spacing)
                        new_start = (0 , Center - new_width+shift)
                        new_end = (grid_x2+width_incr_V , new_width + Center+shift)
...
num_layers=2
new_widths = [3, 5 , 3, 3.5 , 3, 3, 3, 3, 6 ,4]
