...
for source_node in source_nodes:
    for destination_node in destination_nodes:
        ...
        print(paths)   # calculate current in each path
        I_paths = []
        ...
        # calculatin EM violation for the worst path only  
        '''
        for l in range(len(paths[i_worst_index]) - 1):
            for node2 in I_Max:
                if (paths[i_worst_index][l] == node2[0] and paths[i_worst_index][l+1] == node2[1]) or (paths[i_worst_index][l] == node2[1] and paths[i_worst_index][l+1] == node2[0]):
                    if i_worst >= node2[2]:
                        #required_width = ((I_paths[k] / 0.9) + 0.002) / 0.9
                        #required_widths.append(required_widths,(path[l], path[l+1],required_width))
        '''
                ...
                if node[2] >= node2[2]: # if i > imax
                    ...
                    for via in Vias_connected :           
                        if  (node[0] == via[0] and node[1] == via[1]):
                            ...
                            print ("Number of vias",numOfVias)
                            for wire_idx , wire in enumerate(wires):
                                if wire[0][0] <= x1 <= wire[1][0] and wire[0][1] <= y1 <= wire[1][1] and z1==wire[2]:
                                    # Calculate the corresponding width
...
'''
for l in range(len(paths[i_worst_index]) - 1):
    for node2 in required_widths:
        if (paths[i_worst_index][l] == node2[0] and paths[i_worst_index][l+1] == node2[1]) or (paths[i_worst_index][l] == node2[1] and paths[i_worst_index][l+1] == node2[0]):
            print('DONE')
            if i_worst >= node2[2]:
                print(f"EM violation between node {paths[i_worst_index][l]} and {paths[i_worst_index][l+1]} with current {i_worst}A exceeds the maximum allowable current {node2[2]:.4f}A.")
'''
