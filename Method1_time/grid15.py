...
pos = nx.kamada_kawai_layout(G)  # Layout the nodes using the Kamada-Kawai layout algorithm
...
for path in paths:
    ...
    for i in range(len(path) - 1):
        ...
        for j, node2 in enumerate(Via_heights):
            if (path[i] == node2[0] and path[i+1] == node2[1]) or (path[i] == node2[1] and path[i+1] == node2[0]):
...
for j, path in enumerate(paths):
    for i in range(len(path) - 1):
        for node2 in I_Max:
            if (path[i] == node2[0] and path[i+1] == node2[1]) or (path[i] == node2[1] and path[i+1] == node2[0]):
                if I_paths[j] >= node2[2]:
                    print('EM Violation between node', path[i], 'and', path[i+1], 'with current', I_paths[j], 'exceeding maximum', node2[2])
